...
const CreateCategory = () => {
  ...
  const { mutate: createCategory, error: err } = useCreateCategory(onSuccess, onError);
  ...
  const validationObject = yup.object({
    title: yup.string().min(3, "title should atleast 3 characters long").required(),
    desc: yup.string().min(20, "description should be atleast 20 characters long").required(),
  })
  ...
  return (
    <div className="flex flex-col sm:flex-row min-h-screen gap-10 text-xs max-w-[1200px] w-fit mb-20 items-center sm:items-start">
      ...
      <div className={`flex gap-10 justify-center overflow-hidden relative  ${!images ? "items-center" : ""} mt-10 flex-col h-fit w-fit`}>
        <div className=" bg-white p-2 opacity-80 flex items-center gap-4 border-b border-green-500 w-fit">
          <p>Upload images</p>
          <IconButton>
            <input type="file" onChange={imageHandler} className=" w-10 absolute opacity-0"/>
            <PhotoCamera/>
          </IconButton>
        </div>
