...
const UpdateAccount = ({ product, setEditing, setSelectedProduct }) => {
  ...
  const { mutate: updateProduct, error: err } = useUpdateProduct(
    product._id,
    onSuccess,
    onError
  );
  ...
  const initialValues = {
    ...
    size: product.size.map((item) => item),
    color: product.color.map((item) => item),
  ...
  return (
    <div className="flex min-h-screen gap-10 text-normal w-full justify-evenly mb-20">
                  ...
                  <div className=" flex gap-4">
                    ...
                    >
                      ...
                      {collections
                        ?.filter(
                          (collection) =>
                            collection.category === props.values.category
                        )
      ...
      <div
        className={`flex justify-evenly w-full overflow-hidden relative items-start  ${
          !images ? "items-center" : ""
        } mt-10 h-fit w-fit`}
      >
