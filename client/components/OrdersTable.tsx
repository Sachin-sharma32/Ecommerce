...
const OrdersTable = ({ orders, admin, status }: AppProps) => {
  ...
  if (filterOrders?.length === 0) {
    return <MessageModel>There are no orders to show</MessageModel>;
  }

  return (
    <div className="text-[10px] sm:text-normal flex flex-col gap-4 p-4">
      {filterOrders?.map((order) => (
        <div key={order._id} className="border flex flex-col justify-between">
          <div className="grid grid-cols-3 gap-1 md:gap-10 p-2">
            <div className="flex flex-col gap-2 w-fit">
              {order.products.map((product) => (
                <div
                  key={product._id}
                  className=" flex items-center gap-2 relative orderProduct cursor-pointer w-fit h-fit"
                >
                  <img
                  ...
                  <div className="absolute top-7 bg-white shadow-2xl p-2 border-2 orderDetails w-40 z-40">
                    ...
                    <p className="flex">
                      <div className=" w-20">Price</div> Rs.{" "}
                      {product.product.discountPrice
                        ? product.product.discountPrice
                        : product.product.price}
                    </p>
                    <p className="flex">
                      <div className=" w-20">Quantity</div> {product.quantity}
                    </p>
                    <p className="flex">
            ...
            <div className=" w-full">
              <p className="flex">
                <div className="w-20">Line 1</div>
                <span>{order.address.line1}</span>
              </p>
              <p className="flex">
                <div className="w-20">Line 2</div>
              ...
              <p className="flex">
                <div className="w-20">Postal Code</div>
            ...
            <div className=" w-full">
              ...
              <p className="flex">
                <div className="w-20">Ordered On</div>
          ...
          <div className="w-full flex justify-evenly border-t">
            {cancelDialog && (
              ...
              >
                ...
                <DialogContent>
                  <DialogContentText>
                    Are you sure you want to cancel your order. Once cancled you
                    won&apos;t be able receiving this product.
                  </DialogContentText>
                </DialogContent>
                <DialogActions>
                  <Button
            ...
            {order.status !== "delivered" && order.status !== "cancled" && (
              <button
            ...
            {order.status === "cancled" && (
              <button
                disabled
                className=" uppercase w-full border-r text-white bg-red-500 transition-all duration-200"
              >
                Order Was Canceled
              </button>
            )}
            ...
            {order.status === "delivered" && (
              <button className=" cursor-default uppercase w-full border-r bg-green-500 transition-all text-white duration-200">
                Order Delivered
              </button>
            )}
            {admin && order.status !== "delivered" && (
              <button
                className=" uppercase w-full border-r hover:bg-gray-500 hover:text-white transition-all duration-200"
                onClick={() => {
                  changeOrderStatus(order.status, order._id);
                }}
              >
                Next Stage
              </button>
