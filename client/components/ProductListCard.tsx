        ...
        <div className=" w-fit p-0">
            ...
            <div className="product-card w-fit text-xs">
                <div className=" h-fit w-fit flex justify-center cursor-pointer hover:opacity-75 flex-col relative">
                    {product.img && (
                        <Link href={`/product/${product._id}`} className=" h-[100%] w-[100%] relative overflox-hidden">
                            <div className="overflow-hidden">
                                <img src={product.coverImg} className=' w-[100%] h-[100%] bg-gray-200 hover:scale-105 transition-all duration-200' alt="img" />
                            </div>
                            ...
                            {product.offer &&
                                <div className=" bg-green-400 opacity-90 text-white w-fit px-4 absolute top-0 left-0">{product.offer}</div>
                            }
                        </Link>
                    )}
                    <div>
                        <p className=" capitalize text-sm">{product.title}</p>
                        <p className=" text-gray-500">{product.desc.slice(0, 30)}....</p>
                        <div className="flex gap-4">
                            <p className={`text-sm font-semibold ${product.discountPrice ? 'line-through text-gray-400' : ''}`}> &#8377; {product.price}</p>
                            {product.discountPrice && <p className='text-sm font-semibold'> &#8377; {product.discountPrice}</p>}
                        </div>
                    </div>
                </div>
            </div>
