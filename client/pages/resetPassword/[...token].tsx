...
const SignInUser = () => {
  ...
  const { mutate: forgetPassword, error: serverError } = useMutation('forgetPassword', (values) => {
    return axios.post(`http://localhost:8000/api/v1/auth/resetPassword/${token}`, values)
  }, {
    onSuccess: onSuccess,
    onError: onError
  })
  ...
  const validationObject = yup.object({
    password: yup.string().matches(PASSWORD_REGEX, 'please provide a stronger password').required(),
  ...
  return (
    <Smooth className=" min-h-screen flex justify-center items-center bg-gradient-to-r from-gray-100 to-gray-200 text-gray-500 text-xs">
      ...
      <div className="mt-[4rem] grid grid-cols-1 lg:grid-cols-2 form__background  h-fit shadow-2xl justify-items-center min-w-[70vw] max-w-[90%]">
        {error &&
          <ErrorModel>{serverError.response.data.message}</ErrorModel>
        }
        {success &&
          <SuccessModel>Password Changed Successfully</SuccessModel>
        }
            ...
            <div className=" grid grid-cols-1 gap-6 gap-y-2">
              <div className=" flex flex-col items-start ">
                ...
                <input
                  ...
                  required
                />
                <Error className={formik.errors.password ? "opacity-100" : 'opacity-0'}>{formik.errors.password && formik.touched.password ? formik.errors.password : ""}</Error>
              </div>
              <div className=" flex flex-col items-start ">
                <label htmlFor="email" className=" text-md">
                  Password Confirm
                ...
                <input
                  ...
                  required
                />
                <Error className={formik.errors.passwordConfirm ? "opacity-100" : 'opacity-0'}>{formik.errors.passwordConfirm && formik.touched.passwordConfirm ? formik.errors.passwordConfirm : ""}</Error>
              </div>
            </div>
            <button
