...
const CreateCategory = () => {
  ...
  const { mutate: createTestimonial, error: err } = useCreateTestimonial(onSuccess, onError);
  ...
  const validationObject = yup.object({
    message: yup.string().min(20, "message should be atleast 20 characters long").required(),
  })
  ...
  return (
    <div className="flex flex-col sm:flex-row min-h-screen gap-10 text-xs justify-center items-center mb-20 mx-auto bg-gradient-to-r from-gray-50 to-gray-100">
      {success && <SuccessModel>Thank You For Sharing Your Story</SuccessModel>}
      {error && <ErrorModel>{err.response.data.message}</ErrorModel>}
      <div className=" flex flex-col gap-10 w-[500px] items-center">
        <div>
        <h2 className=" text-xl font-semibold text-center">Your Story</h2>
        <p className=" text-gray-500">Help our business grow by sharing your experience shopping with us.</p>
        </div>
        <Formik
          ...
          validationSchema={validationObject}>
          {
            (props) => {
              return (
                <Form
                  className=" flex flex-col justify-center items-center gap-4 w-[500px]"
                >
                  <div className=" w-full">
                    <Field
                      as="textarea"
                      type="text"
                      rows={5} minLength={10} type="text"
                      className=" bg-white outline-none p-2 resize-none rounded-sm focus:shadow-xl w-[500px] transition-all duration-500 border-b-4 border-transparent focus:border-gray-500 focus:invalid:border-red-500"
                      placeholder="Story"
                      name="message"
                    />
      ...
      <div className={`flex gap-10 overflow-hidden relative  mt-10 flex-col min-h-[300px] min-w-[300px]`}>
